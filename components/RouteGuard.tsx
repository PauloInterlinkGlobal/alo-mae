...
export const RouteGuard: React.FC<RouteGuardProps> = ({ children, allowedRoles }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const targetRole = allowedRoles.includes('instituicao') || allowedRoles.includes('admin')
        ? 'instituicao'
        : allowedRoles.includes('professor')
        ? 'professor'
        : 'pai';
      const { user } = await loginWithGoogle(targetRole);
      const userAccount = {
        ...user,
        id: user.uid,
        ...
        <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl text-center">
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full bg-white hover:bg-slate-50 border border-slate-300 text-slate-800 py-2.5 px-4 rounded-xl font-semibold text-xs sm:text-sm flex items-center justify-center gap-2.5 transition-all shadow-xs active:scale-[0.98] cursor-pointer"
            >
              {isGoogleLoggingIn ? (
