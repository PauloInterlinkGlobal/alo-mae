...
export default function PaiComprovantePage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const { logs, students } = useSystem();

  const currentLog =
    logs.find((l) => l.id === resolvedParams.id) ||
    logs.find((l) => l.receiptCode.includes(resolvedParams.id)) ||
    logs[0];
    ...
    <div className="space-y-4 animate-fade-in max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handlePrint}
            className="p-2 bg-white rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors shadow-xs"
            ...
            onClick={() => {
              if (navigator.share) {
                navigator.share({
                  ...
                  text: `Comprovante de ${currentLog.type === 'entry' ? 'Entrada' : 'Saída'} de ${currentLog.studentName}`,
